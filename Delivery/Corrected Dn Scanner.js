...
frappe.ui.form.on('Delivery Note', {
            ...
            if (row.custom_scanned_rolls_data) {
                try {
                    ...
                    if (Array.isArray(rolls) && rolls.length > 0) {
                        ...
                        // NOTE: batch_no is set server-side (Before Submit script)
                        // to avoid the client-side qty-overwrite trigger.
                    }
                } catch (e) {
    ...
    before_submit(frm) {
        ...
        (frm.doc.items || []).forEach(row => {
            ...
            // Rounded comparison to avoid decimal bugs
...
// ─────────────────────────────────────────────────────────────────────────────
//  SO QTY ADJUSTMENT  (uses Server Script API — works on submitted SOs)
//
//  3 cases:
//    1. net_wt === SO qty          → pass through silently
//    2. net_wt ≠ batch_qty         → BLOCK (wrong qty — data entry error)
//    3. net_wt ≠ SO qty, === batch → show confirm popup → POST to Server Script
//       Server Script uses db_set: SO stays submitted, no linked docs touched
// ─────────────────────────────────────────────────────────────────────────────
function maybe_adjust_so_qty(frm, dn_row, so_name, so_detail, batch_no, net_wt, batch_qty, callback) {
    ...
    // Case 1: exact match — nothing to do
    if (diff === 0) { callback(); return; }

    // Case 2: delivery qty ≠ batch produced qty — block with error
...
<div style="text-align:center;padding:10px 0;">
    ...
    // Case 3: net_wt ≠ SO qty but matches batch produced qty → show popup
...
<div style="text-align:center;padding:6px 0 14px;">
    ...
    let d = new frappe.ui.Dialog({
        ...
        primary_action() {
            ...
            // ── Call Server Script via frappe.call ───────────────────────
            // Frappe Server Script (type=API): the "API Method" field on the
            // Server Script form must be set to "update_so_item_qty".
            // frappe.call method MUST match the "API Method" field exactly.
            // Args are read via frappe.form_dict on the Python side.
            frappe.call({
...
//  Called from before_submit hook when scanned total ≠ SO ordered qty.
//  Shows one popup listing ALL mismatched rows. User confirms → Server Script
//  updates each SO item row via db_set. SO stays submitted, no linked docs
//  touched. After all updates done, user manually submits the DN.
// ─────────────────────────────────────────────────────────────────────────────
function show_before_submit_so_popup(frm, mismatches) {

    // Build table rows for each mismatch
...
<div style="padding:6px 0 14px;">
    ...
    let d = new frappe.ui.Dialog({
        ...
        primary_action() {
            ...
            // Process all mismatches sequentially
                    ...
                    frappe.call({
                        ...
                        callback(r) {
                            if (r.exc || !r.message || !r.message.success) {
                                ...
                                // Also update DN row qty to match actual scanned qty
        ...
        secondary_action() {
            d.hide();
            frappe.show_alert({
                message: `⏭️ Skipped SO update. Submitting DN as-is.`,
                indicator: 'orange'
            }, 4);
            // Re-trigger submit without SO check
...
function process_scan(frm, batch_no) {
    ...
    Object.keys(scanned_rolls).forEach(r_name => {
        if (scanned_rolls[r_name].find(r => r.batch_no === batch_no)) already_scanned_row = r_name;
    });

    if (already_scanned_row) {
        frappe.confirm(`Roll <b>${batch_no}</b> is already scanned.<br><br>Do you want to <b>REMOVE</b> this roll?`, () => {
            scanned_rolls[already_scanned_row] = scanned_rolls[already_scanned_row].filter(r => r.batch_no !== batch_no);
            let new_total = flt((scanned_rolls[already_scanned_row] || []).reduce((s, r) => s + r.net, 0), 3);
            let row_doc = (frm.doc.items || []).find(i => i.name === already_scanned_row);
            if (row_doc) {
                ...
                // 🛑 BUG FIX: Do NOT manipulate 'batch_no' on the row to prevent false standard validations
                /*
                if (scanned_rolls[already_scanned_row].length === 0) {
                    frappe.model.set_value(row_doc.doctype, already_scanned_row, 'batch_no', '');
                } else if (row_doc.batch_no === batch_no) {
                    frappe.model.set_value(row_doc.doctype, already_scanned_row, 'batch_no', scanned_rolls[already_scanned_row][0].batch_no);
                }
                */
            }
    ...
    frappe.db.get_doc('Batch', batch_no).then(batch => {

        let item_code = batch.item;
        let net_wt = flt(batch.custom_net_weight) || 0;
        let gross_wt = flt(batch.custom_gross_weight) || 0;
        let meter = flt(batch.custom_meter) || 0;
        let batch_qty = flt(batch.batch_qty) || net_wt;  // Batch Quantity → DN row Quantity

        // Resolve party code from any available field on the batch
        let party = batch.custom_party_code || batch.party_code || batch.custom_party_code_text
        ...
        // ── KEY FIX: Ignore ERPNext's auto-added blank rows ──────────────────
        let dn_items = (frm.doc.items || []).filter(r => r.item_code && r.item_code.trim());
        let dn_item_codes = dn_items.map(r => r.item_code);

        // ── STEP 3: ROUTE ────────────────────────────────────────────────────
        if (dn_item_codes.includes(item_code)) {
            let matching_dn_row = dn_items.find(r => r.item_code === item_code);
            if (matching_dn_row && matching_dn_row.against_sales_order) {
                ...
                type1_duplicate_check_and_add(frm, batch_no, item_code, net_wt, gross_wt, meter, party);
            }
...
<div style="text-align:center;padding:10px 0;">
    ...
    <p style="font-size:13px;color:#555;margin-top:8px;">
        Please select a <b>Customer</b> on this DN first, or ensure the batch has a party code.
            ...
            let linked_sos = [...new Set(dn_items.filter(r => r.against_sales_order).map(r => r.against_sales_order))];

            if (linked_sos.length > 0) {
                let so_to_use = frm.__type2_so_name || linked_sos[0];
...
// ─────────────────────────────────────────────────────────────────────────────
//  TYPE 1 — Duplicate DN check then add roll
// ─────────────────────────────────────────────────────────────────────────────
function type1_duplicate_check_and_add(frm, batch_no, item_code, net_wt, gross_wt, meter, party) {
    frappe.call({
        ...
        callback(r) {
            let existing = (r.message || []).filter(dn => dn.name !== frm.doc.name);

            if (existing.length > 0) {
                let dn_links = existing.map(e => `<b>${e.name}</b>`).join(', ');
                frappe.msgprint({
...
<div style="text-align:center;padding:10px 0;">
    ...
    <p style="font-size:13px;color:#888;margin-top:10px;">Please scan a different roll.</p>
            ...
            // Type 1 has no SO link — add roll directly (no SO qty adjustment)
...
<div style="text-align:center;padding:10px 0;">
    ...
    function show_so_result(orders) {
        if (orders.length === 1) {
            ...
            let so_options = orders.map(o =>
                `<option value="${o.name}">${o.name} &nbsp;|&nbsp; ${o.customer} &nbsp;|&nbsp; ${o.transaction_date} &nbsp;|&nbsp; Delivery: ${o.delivery_date || '—'}</option>`
            ).join('');
...
<select id="type2_so_picker" style="width:100%;padding:10px;font-size:14px;border:1px solid #ccc;border-radius:4px;">
    ${so_options}
                ...
                primary_action() {
                    let selected = $('#type2_so_picker').val();
                    d.hide();
                    if (selected) {
    ...
    if (filters_by_party) {
        do_so_search(filters_by_party, filters_by_customer ? () => do_so_search(filters_by_customer, null) : null);
    } else {
...
function type2_create_row_for_so(frm, so_name, batch_no, item_code, net_wt, gross_wt, meter, party, batch_qty) {
    ...
    frappe.call({
        ...
        callback(dup_r) {
            let existing = (dup_r.message || []).filter(dn => dn.name !== frm.doc.name);
...
<div style="text-align:center;padding:10px 0;">
    ...
    <p style="font-size:13px;color:#888;margin-top:10px;">Please scan a different roll.</p>
            ...
            frappe.db.get_value('Sales Order', so_name, ['custom_party_code', 'customer'], so_vals => {
                let so_party = (so_vals && so_vals.custom_party_code) ? so_vals.custom_party_code.trim() : '';
                let so_customer = (so_vals && so_vals.customer) ? so_vals.customer : '';
...
<div style="text-align:center;padding:10px 0;">
    ...
    <p style="font-size:13px;color:#888;margin-top:10px;">Please scan a roll that belongs to party <b>${so_party}</b>.</p>
                ...
                // ── Row already exists → SO qty check then add roll ───────────
                let existing_row = (frm.doc.items || []).find(r =>
                    r.item_code === item_code && r.against_sales_order === so_name
                );

                if (existing_row) {
                    // No SO popup on scan — check happens at before_submit
                    add_roll_to_form(frm, batch_no, item_code, net_wt, gross_wt, meter, party);
                    return;
                }
                ...
                frappe.db.get_doc('Sales Order', so_name).then(so => {

                    let so_item = (so.items || []).find(i => i.item_code === item_code);
...
<div style="text-align:center;padding:10px 0;">
    ...
    <p style="font-size:13px;color:#555;margin-top:8px;">
        ...
        Please check the correct Sales Order for this roll.
                    ...
                    (frm.doc.items || []).filter(r => !r.item_code || !r.item_code.trim()).forEach(blank => {
                        let gr = frm.fields_dict.items.grid.grid_rows_by_docname[blank.name];
                        if (gr) gr.remove();
                    });
                    ...
                    let _wh_so_item = so_item.warehouse || so.set_warehouse || '';

                    frappe.db.get_doc('Item', item_code).then(item_doc => {
                        let item_def = (item_doc.item_defaults || []).find(d => d.company === frm.doc.company);
...
function add_roll_to_form(frm, batch_no, item_code, net_wt, gross_wt, meter, party) {
    ...
    for (let row of (frm.doc.items || [])) {
        ...
        let already_delivered = (scanned_rolls[row.name] || []).reduce((s, r) => s + r.net, 0);
    ...
    let ordered_qty = flt(matching_row.qty);

    if (!scanned_rolls[row_name]) scanned_rolls[row_name] = [];
    scanned_rolls[row_name].push({ batch_no, net: net_wt, gross: gross_wt, meter, party });

    let total_delivered = flt(scanned_rolls[row_name].reduce((s, r) => s + r.net, 0), 3);
    set_delivered_safe(frm, row_name, matching_row.doctype, total_delivered);
    save_rolls_data(frm, row_name, matching_row.doctype);

    // NOTE: batch_no is NOT set here.
    // Setting batch_no client-side triggers ERPNext's default handler which
    // overwrites row qty with single batch stock qty — even with our override.
    // batch_no is set server-side in the Before Submit script using
    // Serial and Batch Bundle (ERPNext v14+ multi-batch approach).


    let remaining = flt(ordered_qty - total_delivered, 3);
...
function add_rolls_buttons(frm) {
    ...
    frm.fields_dict.items.grid.grid_rows.forEach(row => {
        ...
        let rolls = scanned_rolls[doc.name] || [];
        let ordered_qty = flt(doc.qty || 0);
        let scanned_wt = flt(rolls.reduce((s, r) => s + r.net, 0), 3);
        let count = rolls.length;

        let btn_color = '#555';
        let status_icon = '';

        // ✅ Improved Color Logic (Tolerance Safe)
        let tolerance = 0.0001;

        if (count > 0) {

            if (Math.abs(scanned_wt - ordered_qty) <= tolerance) {
                btn_color = '#1a7a3c';   // Green
                status_icon = ' ✅';
            }
            else if (scanned_wt > ordered_qty) {
                btn_color = '#b00020';   // Red
                status_icon = ' 🔼';
            }
            else {
                btn_color = '#c07a00';   // Orange
                status_icon = ' ⚠️';
            }
        }
...
function show_rolls_popup(frm, row_doc) {
    let row_name = row_doc.name;
    let rolls = scanned_rolls[row_name] || [];
    let ordered_qty = flt(row_doc.qty);
    ...
    let total_gross = flt(rolls.reduce((s, r) => s + r.gross, 0), 3);
    let total_meter = flt(rolls.reduce((s, r) => s + r.meter, 0), 3);
    let total_count = rolls.length;
...
<tr>
    ...
    <td style="padding:10px;border:1px solid #d1d8dd;text-align:center;">${r.batch_no}</td>
    ...
    <td style="padding:10px;border:1px solid #d1d8dd;text-align:center;">${r.party || '—'}</td>
</tr>`).join('');

    let html = `
<style>
  /* Custom CSS to expand the frappe dialog width */
  .modal-dialog {
...
<table style="width:100%;border-collapse:collapse;font-size:13px;border:1px solid #d1d8dd;text-align:center;">
<thead><tr style="background-color:#f8f9fa;">
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;white-space:nowrap;text-align:center;width:40px;">#</th>
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;white-space:nowrap;text-align:center;">Batch No</th>
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;text-align:center;white-space:nowrap;">Net Weight (kgs)</th>
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;text-align:center;white-space:nowrap;">Gross Weight (kgs)</th>
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;text-align:center;white-space:nowrap;">Meter / Roll (mtr)</th>
    <th style="padding:10px;border:1px solid #d1d8dd;font-weight:bold;white-space:nowrap;text-align:center;">Party Code</th>
</tr></thead>
<tbody>${rows_html}</tbody>
<tfoot><tr style="background-color:#e8f4fd;font-weight:bold;font-size:13px;text-align:center;">
    <td style="padding:10px;border:1px solid #b8daff;"></td>
    <td style="padding:10px;border:1px solid #b8daff;color:#0056b3;white-space:nowrap;">🧮 Total (${total_count} Rolls)</td>
    <td style="padding:10px;border:1px solid #b8daff;color:#1a7a3c;">${total_net} kgs</td>
    <td style="padding:10px;border:1px solid #b8daff;color:#555;">${total_gross} kgs</td>
    <td style="padding:10px;border:1px solid #b8daff;color:#555;">${total_meter} mtr</td>
    <td style="padding:10px;border:1px solid #b8daff;"></td>
</tr></tfoot>
</table>
</div>
<p style="text-align:right;font-size:11.5px;color:#666;margin-top:15px;margin-bottom:0;">
