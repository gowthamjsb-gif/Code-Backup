                ...
                for (let row of frm.doc.items) {
                    if (isBagItem(row.item_code)) {
                        ...
                        if (calc_gsm > 0 && calc_width > 0) {
                            // Find the best no_of_rolls such that final weight_per_roll is closest to 120
                            let best_n = 1;
                            let min_diff = Infinity;
                            for (let n = 1; n <= 100; n++) {
                                let test_meter_per_roll = (actual_meter / n) + extra_meter;
                                let test_weight = (calc_gsm * calc_width * test_meter_per_roll * 0.0254) / 1000;
                                let diff = Math.abs(test_weight - 120);
                                if (diff < min_diff) {
                                    min_diff = diff;
                                    best_n = n;
                                } else {
