(function () {
    ...
    function isLhMetallicBoppSlittedProcess(row) {
        return String(row && row.custom_lh_process || "").trim()
            === "NON WOVEN METTALIC BOPP LAMINATED SLITTED FABRIC";
    }

    function isLhBoppSlittedProcess(row) {
        const p = String(row && row.custom_lh_process || "").trim();
        return p === "NON WOVEN BOPP LAMINATED SLITTED FABRIC"
            || p === "NON WOVEN METTALIC BOPP LAMINATED SLITTED FABRIC";
    }

    function findLhGsmOption(frm, fieldname, targetGsm) {
        let df = frappe.meta.get_docfield("Quotation Item", fieldname, frm.doc.name);
        if (df && df.options) {
            for (let o of df.options.split("\n")) {
                if (parseGSMValue(o) === targetGsm) {
                    return o.trim();
                }
            }
        }
        return String(targetGsm);
    }

    function applyLhBoppGsmSplit(frm, cdt, cdn, row) {
        row = row || (locals[cdt] && locals[cdt][cdn]);
        if (!row || !isLhBoppSlittedProcess(row)) return;
        let total = parseFloat(row.custom_loop_handle_gsm) || 0;
        if (total <= 0) return;
        const isMetallic = isLhMetallicBoppSlittedProcess(row);
        const lamGsm = 15;
        const boppGsm = isMetallic ? 30 : 15;
        const fabGsm = total - lamGsm - boppGsm;
        const lam_opt = findLhGsmOption(frm, "custom_lh_lamination_gsm", lamGsm);
        const bopp_opt = findLhGsmOption(frm, "custom_lh_bopp_gsm", boppGsm);
        const fab_opt = findLhGsmOption(frm, "custom_lh_fabric_gsm", fabGsm);
        if (row.custom_lh_lamination_gsm !== lam_opt) {
            frappe.model.set_value(cdt, cdn, "custom_lh_lamination_gsm", lam_opt);
        }
        if (row.custom_lh_bopp_gsm !== bopp_opt) {
            frappe.model.set_value(cdt, cdn, "custom_lh_bopp_gsm", bopp_opt);
        }
        if (row.custom_lh_fabric_gsm !== fab_opt) {
            frappe.model.set_value(cdt, cdn, "custom_lh_fabric_gsm", fab_opt);
        }
    }

    function applyLhBoppCoatingDefaults(frm, cdt, cdn, row) {
        row = row || (locals[cdt] && locals[cdt][cdn]);
        if (!row || !isLhBoppSlittedProcess(row)) return;
        if (isLhMetallicBoppSlittedProcess(row)) {
            frappe.model.set_value(cdt, cdn, "custom_coating", "Mettalic");
            frappe.model.set_value(cdt, cdn, "custom_finishing", "Matte");
        } else {
            frappe.model.set_value(cdt, cdn, "custom_coating", "Plain");
            frappe.model.set_value(cdt, cdn, "custom_finishing", "Matte");
        }
    }
    ...
    frappe.ui.form.on("Quotation Item", {
        ...
        custom_lh_process: function (frm, cdt, cdn) {
            const row = locals[cdt] && locals[cdt][cdn];
            if (!row) return;
            applyLhBoppCoatingDefaults(frm, cdt, cdn, row);
            if (parseFloat(row.custom_loop_handle_gsm) > 0) {
                applyLhBoppGsmSplit(frm, cdt, cdn, row);
            }
        },
        custom_loop_handle_gsm: function (frm, cdt, cdn) {
            const row = locals[cdt][cdn];
            if (row && isLhBoppSlittedProcess(row)) {
                applyLhBoppGsmSplit(frm, cdt, cdn, row);
            }
        },
        custom_lh_fabric_gsm: function (frm, cdt, cdn) {
            ...
            if (row && isLhBoppSlittedProcess(row)) {
                ...
                let bopp = parseGSMValue(row.custom_lh_bopp_gsm) || (isLhMetallicBoppSlittedProcess(row) ? 30 : 15);
                if (fabric > 0) {
