...
const COMPANY_PRINT_FORMAT = {
    "Jayashree Spun Bond - 1ZT": "Final Quotation Template",
    "Jayashree Spun Bond - 2ZS": "Quotation - Export",
};
...
frappe.ui.form.on("Quotation", {
    setup(frm) {
        frm.set_query("print_format", function () {
            ...
            const format_name = COMPANY_PRINT_FORMAT[company];
            if (format_name) {
...
function set_print_format_by_company(frm) {
    ...
    const format_name = COMPANY_PRINT_FORMAT[frm.doc.company];
    if (format_name) {
...
function apply_quotation_print_view_patch() {
    ...
    PV.prototype.set_default_print_format = function () {
        ...
        if (
            ...
            const mapped = COMPANY_PRINT_FORMAT[this.frm.doc.company];
